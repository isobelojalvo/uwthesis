...
Three gauge fields $W_{\mu}^{a}$ with a = 1,2,3 are put in place for the three gauge bosons, $W^{\pm}$ and Z. %%check
...
\label{eq:L2}
\end{equation}
If we wish for \ref{eq:L2} to remain gauge invariant and 
for this Lagrangian to break the symmetries of $SU(2)$ and $U(1)_{Y}$
but leave $U(1)_{em}$ with generator, $Q=T^{3}+\frac{Y}{2}$, unbroken then
...
\begin{array}{c}
    ...
    \end{array}
\end{equation}
Again, we are interested in the case where $\mu^{2}<0$ and $\lambda>0$
and choose a point in $\phi(x)$ that is a global minimum,
\begin{equation}
